...
## 🔧 How this is implemented in YouTube Truth Panel

The matrix above is enforced in **`server.py`**, not left to the model alone:

| Piece | Where | Behaviour |
| :--- | :--- | :--- |
| Domain → level lookup | `TRUST_DOMAINS`, `TRUST_TLDS` | Every citation URL is classified by hostname; TLD rules (`.gov`, `.edu`, `.mil`, `.int`, `.ac.uk`, `.gov.uk`) apply when no explicit domain matches. |
| Prompt constraint | `FACTCHECK_PROMPT` | Sends the hierarchy to the Research/MCP call and asks for `VERDICT: X (Trust Level: N)`. |
| Model's self-report | `_parse_verdict` | The `(Trust Level: N)` it returns is captured as `model_trust_level` — recorded, but the server's own classification is what the UI shows. |
| Level 3 rule | `_apply_trust_rules` | A `TRUE`/`FALSE` verdict backed **only** by Level 3+ sources is downgraded to `UNVERIFIED`. |
| Level 4 rule | `_apply_trust_rules` | A verdict backed **only** by Level 4 sources is downgraded to `UNVERIFIED` and flagged. |
| Level 1 rule | `_apply_trust_rules` | A single Level 1 citation with no Level 0 support is **noted** (`below the 2-source minimum`) but not downgraded — see caveat below. |
| Display | `sidepanel.js` / `.css` | Sources are sorted best-first and every link is prefixed with an `L0`–`L4` badge, colour-coded, with the category name on hover. |

**Unrated domains** (anything not in the lists) default to **Level 2** and are marked
`unrated` in the tooltip, so a default is never mistaken for a vetted classification.

**Caveat on the Level 1 two-source minimum:** enforcing it as a hard downgrade
would turn many well-sourced verdicts into `UNVERIFIED`, since the Research API
often returns a single strong citation. It is therefore surfaced as a note
rather than a downgrade. To make it strict, extend `_apply_trust_rules`.

The domain lists are deliberately short and sit at the top of `server.py` for
easy editing — treat them as a starting set, not a complete registry.
